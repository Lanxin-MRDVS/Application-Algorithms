...
[Documentation Home](../../../README.md) / [LxCameraViewer](../README.md) / User Guide
...
3. Select the installation language (see Figure 2.2)
4. Select the installation path (using the default path is recommended) (see Figure 2.3)
5. Check **Create a desktop shortcut** (see Figure 2.4)
6. After installation completes, uncheck **Launch Lanxin-MRDVS** and click **Finish** (see Figure 2.5)
7. The first time you launch the software, run it as administrator. Right-click the desktop shortcut and select **Run as administrator** (see Figure 2.6)
...
4. If a window like Figure 2.10 appears, the firewall is on. If a window like Figure 2.11 appears, the firewall is off.
...
- The device list is located in the top-left corner of the interface (Area 1, red in Figure 3.3)
- The **Refresh** button is next to the device list (Area 2, red in Figure 3.4)
- The collapse/expand button is to the right of the Refresh button (Area 2, green in Figure 3.4). When collapsed, the list appears as shown in Figure 3.5
- The device list displays three properties:
  - Camera connection status
  - Camera ID (Area 2, yellow in Figure 3.4)
  - Camera IP (hover over the Camera ID to display the IP, shown in the red box in Figure 3.6)
- Each camera in the list has an **[Open/Close]** button to open or close the camera (blue box in Figure 3.4). The button also displays the current camera status (Figure 3.7)
- The selected camera is highlighted in blue; unselected cameras appear in white (Figure 3.8 shows the second camera selected but not opened, and the first camera opened but not selected)
- Multiple devices can be opened simultaneously (Figure 3.9). Ensure sufficient network bandwidth. Click a camera ID to switch between cameras
...
- Current information is located in the middle-left area of the interface (Area 1, red in Figure 3.10)
- It displays the following:
  - All camera information (Area 2, red in Figure 3.4)
  - Software operations (Area 2, green in Figure 3.4)
  - Operation results (Area 2, blue in Figure 3.4)
  - Streaming results (Area 4, magenta in Figure 3.6)
- When text overflows, drag the scrollbar at the bottom to view the full content (Figure 3.5 shows an additional scrollbar at the bottom)
...
- Click the collapse button to collapse this panel (Area 2, yellow in Figure 3.11)
...
- Device information is located in the bottom-left corner of the interface (Area 1, red in Figure 3.10)
...
- Click the collapse button to collapse this panel (Area 2, yellow in Figure 3.15)
- Right-click a specific item to copy its value (Figure 3.16 shows copying network speed information)
...
- Image data and related operations are located in the center of the interface (Area 1, red in Figure 3.17)
...
- The white text at the bottom displays camera and image data (Figure 3.18)
...
- **[Save Single Image]**: Saves the currently displayed image data
  - For example, in Figure 3.18, only the depth and point cloud images are saved; the RGB and amplitude images are not enabled and therefore not saved
...
- In Figure 3.19, the device is streaming and displaying depth, point cloud, RGB, and amplitude images
- In Figure 3.20, the device has stopped streaming and no images are displayed
...
- Displays the camera temperature (Area 4, red in Figure 3.21)
- When the depth image is selected, shows the depth image frame rate (Area 4, green in Figure 3.21)
- When the amplitude image is selected, shows the amplitude image frame rate (Area 4, blue in Figure 3.21)
- When the RGB image is selected, shows the RGB image frame rate (Area 4, magenta in Figure 3.21)
- When the depth image is selected, shows the pixel values of the selected pixels in the depth image (Area 4, yellow in Figure 3.21)
- When the amplitude image is selected, shows the pixel values of the selected pixels in the amplitude image (Area 4, orange in Figure 3.21)
- When the point cloud image is selected, shows the point cloud values corresponding to the selected pixels in the depth image (Area 4, black in Figure 3.21)
...
- **Single Frame**: Displays one image when one of the four types is selected (Figure 3.22)
- **Dual Frame**: Displays two images when two types are selected (Figure 3.23)
- **Triple Frame**: Displays three images when three types are selected (Figure 3.24)
- **Quadruple Frame**: Displays all four images when all types are selected (Figure 3.25)

Mouse operations:
- Scroll the mouse wheel to zoom in/out, centered on the cursor position
- Hold the left mouse button and drag to pan
- Left-click to select a pixel; use arrow keys to move the selection (Figure 3.23)
...
The menu bar includes **[Basic]**, **[Algorithms]**, **[Others]**, and **[Language]** (Figure 3.26).

- Clicking **[Basic]** displays the options shown in Figure 3.27
- Clicking **[Algorithms]** displays the options shown in Figure 3.28
- Clicking **[Others]** displays the options shown in Figure 3.29
- Clicking **[Language]** displays the options shown in Figure 3.30
...
- Collapse/expand the right sidebar (Area 1, red in Figure 3.31)
- **Expand All** and **Collapse All** buttons (Area 1, blue in Figure 3.31; upper icon collapses all, lower icon expands all)
- Click a function name to display the corresponding SDK setting information (highlighted in blue at the bottom of Figure 3.31)
...
- Expanded and collapsed states are shown in Figure 3.31 (2D Settings is expanded; others are collapsed)
...
Clicking **[Basic]** on the menu bar displays the basic tools settings on the right side of the interface (Figure 3.31).

Basic tools include **[Image Display Settings]**, **[2D Settings]**, **[3D Settings]**, **[Filtering]**, **[Hardware Interface]**, **[Collection Settings]**, and **[Function Settings]** (Figure 3.33). Function names and enumeration values are displayed in the blue box at the bottom (Figure 3.33).
...
| Setting                             | Description                                                          |
| ----------------------------------- | -------------------------------------------------------------------- |
| **[Display Camera Model]**         | Toggle camera model overlay on point cloud (Figures 3.35–3.36)             |
...
| **[Cloud Save Method]** | **Normal** (Unordered Point Cloud): point count unchanged, width = width × height, height = 1 (Figure 3.45). **Ordered Point Cloud**: point count unchanged, width = ToF width, height = ToF height (Figure 3.46) |
...
Clicking **[Algorithms]** on the menu bar displays the algorithm settings on the right side of the interface (Figure 3.43).
...
- **[Algorithm]**: Select the algorithm to enable:
  - **[Close]** (Figure 3.44)
  - **[Obstacle Avoidance settings 2.0]** (Figure 3.45)
  - **[Tray Identification settings]** (Figure 3.46)
...
| :---: | :---: | :---: |
| *Figure 3.44* | *Figure 3.45* | *Figure 3.46* |
...
- Connect to the camera IP address via SSH on port 22
  - Example using MobaXterm with camera IP `192.168.100.15` (Figures 4.1 and 2)
- Username: `user`, Password: `user`
- After successful login, the interface shown in Figure 4.3 will be displayed
...
- Open **[Camera IP Configuration Tool]**; the interface is shown in Figure 5.1
...
- Click elsewhere to cancel modifications; press Enter to confirm (Figure 5.2)
...
- Open **[Communication Tool]**; the interface is shown in Figure 5.3
